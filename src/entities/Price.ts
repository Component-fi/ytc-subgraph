...
export function logPrices(timestamp: BigInt): void {
  log.info("Logging prices", [timestamp.toString()]);
...
function addPrice(feedId: string, timestamp: BigInt): Price {
  ...
  let priceResult = feedRegistry.try_latestAnswer(baseAddress, quoteAddress);

  if (!priceResult.reverted) {
    price.price = priceResult.value;
  }

  let decimalsResult = feedRegistry.try_decimals(baseAddress, quoteAddress);
  ...
  log.info("Adding a price {}", [feedId + timestamp.toString()]);
